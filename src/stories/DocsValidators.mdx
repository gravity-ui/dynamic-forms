...
4. [Validation of Numbers with Scale](#4-validation-of-numbers-with-scale)
5. [Validation of Objects](#5-validation-of-objects)
6. [Validation of Strings](#6-validation-of-strings)
7. [Adding Custom Validation](#7-adding-custom-validation)
...
## 4. Validation of Numbers with Scale

Some numeric fields are stored in a single base unit but entered by users in a more convenient, scaled unit. A typical example is a duration stored in milliseconds but displayed and entered in seconds, minutes, hours, or days. These fields use the `number_with_scale` view and the matching `number_with_scale` validator.

The validator performs all the same checks as the [number validator](#3-validation-of-numbers) (required field, spaces, numeric correctness, leading/trailing zeros, dot at the end, integer check, `maximum`/`minimum`). The key difference is in how the `maximum` and `minimum` errors are reported: instead of always showing the raw limit in the base unit, the validator picks the most human-readable scale for that limit and reports it together with the scale title.

For example, with a limit of `3600000` milliseconds, the error message will read `Value must not be greater than 1 hours` rather than `Value must not be greater than 3600000`. If a readable scale cannot be derived (for instance when `sizeParams` is not provided), the validator falls back to the plain numeric message used by the number validator.

### Example Usage in Specification:

The `sizeParams` block describes the available scales. Each scale entry has a `factor` (how many base units it represents, as a string to support very large values) and a `title` (the label shown in the UI and in error messages). `defaultType` is the base unit the value is stored in, and `viewType` is the scale initially shown to the user.
...
### Implementation of the Number with Scale Validator
...
export const getNumberWithScaleValidator = (params: GetNumberWithScaleValidatorParams = {}) => {
    ...
    return (spec: StringSpec, value = '') => {
        ...
        // ... the same required / space / number / zero checks as getNumberValidator ...
...
**Explanation:**
    ...
    -   **Inherited checks**: The validator accepts the same `GetNumberValidatorParams` and runs the same checks as the [number validator](#3-validation-of-numbers).
    -   **`getScaledLimit(spec, limit)`**: Reads `spec.viewSpec.sizeParams`, finds the largest scale in which the limit is still "readable" (its integer part is at least `1`), converts the limit into that scale, and returns `{count, scaleTitle}`.
    -   **Scaled error messages**: When a scaled limit is available, the validator reports `maxNumberWithScale` / `minNumberWithScale` (for example, `Value must not be greater than 1 hours`). Otherwise it falls back to `maxNumber` / `minNumber` with the raw value.
    -   The validator returns an error string if validation fails, or `false` if everything is correct.

---

## 5. Validation of Objects
